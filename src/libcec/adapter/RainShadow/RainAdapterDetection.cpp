...
#include "env.h"
#include "RainAdapterDetection.h"
...
bool CRainAdapterDetection::CanAutodetect(void)
{
...
uint8_t CRainAdapterDetection::FindAdaptersWindows(cec_adapter_descriptor* deviceList, uint8_t iBufSize, const char* strDevicePath /* = NULL */)
{
  uint8_t iFound(0);
  ...
  if (buffer)
  {
    ...
    for (CHAR* devId = buffer; *devId; devId += strlen(devId) + 1)
    {
      ...
      // no match
      if ((iVendor != RAIN_CEC_VID || iProduct != RAIN_CEC_PID) && (iVendor != RAIN_CEC_VID2 || iProduct != RAIN_CEC_PID2))
        continue;
...
uint8_t CRainAdapterDetection::FindAdaptersApple(cec_adapter_descriptor *deviceList, uint8_t iBufSize, const char *strDevicePath /* = NULL */)
{
  uint8_t iFound(0);
    ...
    if (kresult == KERN_SUCCESS)
    {
      ...
      while ((serialService = IOIteratorNext(serialPortIterator)))
      {
        ...
        if (bsdPathAsCFString)
        {
          ...
          if (strlen(bsdPath) && (iVendor == RAIN_CEC_VID && iProduct == RAIN_CEC_PID || iVendor == RAIN_CEC_VID2 && iProduct == RAIN_CEC_PID2))
          {
...
uint8_t CRainAdapterDetection::FindAdaptersUdev(cec_adapter_descriptor *deviceList, uint8_t iBufSize, const char *strDevicePath /* = NULL */)
{
  uint8_t iFound(0);
...
uint8_t CRainAdapterDetection::FindAdaptersFreeBSD(cec_adapter_descriptor *deviceList, uint8_t iBufSize, const char *strDevicePath /* = NULL */)
{
  uint8_t iFound(0);
...
uint8_t CRainAdapterDetection::FindAdapters(cec_adapter_descriptor *deviceList, uint8_t iBufSize, const char *strDevicePath /* = NULL */)
{
  uint8_t iFound(0);
